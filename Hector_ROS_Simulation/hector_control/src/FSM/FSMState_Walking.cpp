...
// 目前使用的FSM是这个，用于根据键盘修改机器人状态

FSMState_Walking::FSMState_Walking(std::shared_ptr<ControlFSMData> data)
    : FSMState(data, FSMStateName::WALKING, "walking"), Cmpc(0.001, 40) {}
...
void FSMState_Walking::run() {
    ...
    // std::cout << "vx vy " << v_des_body[0] << " " << v_des_body[1] <<
    // std::endl;
    _data->_desiredStateCommand->setStateCommands(roll, pitch, v_des_body,
                                                  turn_rate);

    Cmpc.setGaitNum(2); // 2 for walking 1 for stand
    Cmpc.run(*_data);
